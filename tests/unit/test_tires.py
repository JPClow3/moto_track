    ...
    def test_list_view_keeps_pagination_controls_for_multiple_pages(self):
        TireRecord.objects.bulk_create(  # pylint: disable=no-member
            [
                TireRecord(
                    motorcycle=self.motorcycle,
                    position=TirePosition.FRONT if index % 2 == 0 else TirePosition.REAR,
                    brand_model=f"Historico {index}",
                    installed_at="2026-03-01",
                    installed_odometer_km=9000 + index,
                    cost="500.00",
                    wear_percent=15,
                    is_active=False,
                )
        ...
        response = self.client.get(reverse("tires:list"), {"page": 2})
