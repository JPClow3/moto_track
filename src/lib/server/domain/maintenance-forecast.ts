...
function dayDifference(start: string, end: string) {
  return Math.max(0, (new Date(`${end}T00:00:00Z`).getTime() - new Date(`${start}T00:00:00Z`).getTime()) / 86_400_000);
}
...
export function forecastMaintenance({
  ...
  const dates = [...odometerSamples, lastService]
  ...
  const kmPerDay = days > 0 ? Math.max(0, (latest.odometer_km - first.odometer_km) / days) : null;
  const dueByKm = intervalKm === null ? null : lastService.odometer_km + intervalKm;
  const dueByDate = intervalDays === null ? null : addDays(lastService.date, intervalDays);
  const predictedDateByKm = dueByKm !== null && kmPerDay && kmPerDay > 0
    ? addDays(today, Math.max(0, (dueByKm - currentKm) / kmPerDay))
    : null;
  const datesToCompare = [dueByDate, predictedDateByKm].filter((date): date is string => Boolean(date));
  return {
    dueKm: dueByKm,
