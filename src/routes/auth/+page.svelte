    ...
    <div class="panel relative overflow-hidden p-6 shadow-lift sm:p-8">
      ...
      <form class="grid gap-5" method="POST" action="?/signIn">
        ...
        <div class="grid gap-1.5">
          <label for="email" class="label-tech text-[var(--muted)]">Email</label>
          <div class="relative">
            <Mail class="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[var(--muted)]" />
            <input id="email" class="field pl-9" name="email" type="email" value={form?.email ?? ''} placeholder="voce@exemplo.com" required />
          </div>
        </div>

        <div class="grid gap-1.5">
          <label for="password" class="label-tech text-[var(--muted)]">Senha</label>
          <div class="relative">
            <KeyRound class="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[var(--muted)]" />
            <input id="password" class="field pl-9" name="password" type="password" placeholder="••••••••" required />
          </div>
        </div>

        <button class="button-primary mt-2 w-full py-2.5 text-base" type="submit">
          <LogIn class="h-4 w-4" />
          Entrar
        </button>
      </form>
      ...
      <div class="relative my-8">
        ...
        <div class="relative flex justify-center">
          <span class="label-tech bg-[var(--panel)] px-4 text-[var(--muted)]">ou continuar com</span>
        </div>
      </div>
      ...
      <details class="group mt-8 rounded border border-[var(--line)] [&_summary::-webkit-details-marker]:hidden">
        <summary class="flex cursor-pointer items-center justify-center gap-2 p-3 text-sm font-semibold transition-colors hover:text-[var(--accent)]">
          Criar conta ou recuperar senha
          <svg class="h-4 w-4 transition-transform group-open:rotate-180" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
        </summary>
        <div class="grid gap-6 border-t border-[var(--line)] p-4">
