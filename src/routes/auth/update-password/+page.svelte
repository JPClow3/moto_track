...
<div class="relative flex min-h-screen items-center justify-center overflow-hidden px-4 py-12">
  ...
  <div class="relative w-full max-w-[440px]">
    <div class="mb-8 text-center">
      ...
      <h1 class="display text-4xl">Defina sua nova senha.</h1>
      <p class="mt-3 text-sm text-[var(--muted)]">
        Escolha uma senha nova para continuar acessando sua garagem.
      </p>
    </div>
    ...
    <div class="panel relative overflow-hidden p-6 shadow-lift sm:p-8">
      ...
      <form class="grid gap-5" method="POST">
        <div class="grid gap-1.5">
          <label for="password" class="label-tech text-[var(--muted)]">Nova senha</label>
          <div class="relative">
            <KeyRound class="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[var(--muted)]" />
            <input id="password" class="field pl-9" name="password" type="password" placeholder="mínimo 6 caracteres" required minlength="6" />
          </div>
        </div>

        <div class="grid gap-1.5">
          <label for="confirmPassword" class="label-tech text-[var(--muted)]">Confirme a senha</label>
          <div class="relative">
            <KeyRound class="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[var(--muted)]" />
            <input id="confirmPassword" class="field pl-9" name="confirmPassword" type="password" placeholder="••••••••" required minlength="6" />
          </div>
        </div>
