<script lang="ts">
  ...
  const price = (millicents: number) =>
    new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL', minimumFractionDigits: 3 }).format(
      millicents / 100000
    );
  $: ocr = form?.ocr;
...
<section class="grid gap-6">
  ...
  <div class="grid gap-4 md:grid-cols-4">
    <article class="panel p-4"><p class="text-sm text-[var(--muted)]">Gasto total</p><strong class="text-2xl">{brl(data.summary.totalSpend)}</strong></article>
    <article class="panel p-4"><p class="text-sm text-[var(--muted)]">Litros</p><strong class="text-2xl">{data.summary.totalLiters.toFixed(2)}</strong></article>
    <article class="panel p-4"><p class="text-sm text-[var(--muted)]">Média</p><strong class="text-2xl">{data.summary.averageConsumption ?? '—'} km/l</strong></article>
    <article class="panel p-4"><p class="text-sm text-[var(--muted)]">Custo/km</p><strong class="text-2xl">{data.summary.costPerKm ? brl(data.summary.costPerKm * 100) : '—'}</strong></article>
  </div>
  ...
  {#if form?.ocr}
    <div class="panel p-4">
      ...
      <p class="mt-2 text-sm text-[var(--muted)]">
        Litros: {form.ocr.liters ?? '—'} · Total: {form.ocr.total_price ? brl(form.ocr.total_price * 100) : '—'} · Preço/l:
        {form.ocr.price_per_liter ? price(form.ocr.price_per_liter * 100000) : '—'}
      </p>
      <p class="mt-2 text-sm text-[var(--muted)]">Os valores foram colocados no novo abastecimento abaixo para revisão.</p>
    </div>
  {/if}
    ...
    <div class="panel p-4">
      ...
      <div class="mt-3 overflow-x-auto">
        <table class="w-full min-w-[720px] text-left text-sm">
          <thead><tr><th>Linha</th><th>Data</th><th>Km</th><th>Litros</th><th>Total</th><th>Status</th></tr></thead>
          <tbody>
            {#each form.previewRows as row}
              <tr class="border-t border-[var(--line)]">
                <td class="py-2">{row.row}</td><td>{row.data.date}</td><td>{row.data.odometer_km}</td><td>{row.data.liters}</td><td>{brl(row.data.total_price_cents)}</td>
      ...
      <form class="mt-4 flex gap-3" method="POST" action="?/importConfirm" use:enhance>
        ...
        <select class="field max-w-xs" name="motorcycle_id">
          ...
          {#each data.motorcycles as moto}<option value={moto.id}>{moto.name}</option>{/each}
        </select>
        <button class="button-primary" type="submit">Importar linhas válidas</button>
      </form>
    </div>
  {/if}
  ...
  <div class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_380px]">
      ...
      <div class="overflow-x-auto">
        <table class="w-full min-w-[820px] text-left text-sm">
          <thead class="border-b border-[var(--line)] text-xs uppercase text-[var(--muted)]">
            <tr><th class="px-4 py-3">Data</th><th>Km</th><th>Litros</th><th>Total</th><th>Preço/l</th><th>Posto</th><th>Ações</th></tr>
          </thead>
          <tbody>
            ...
            {:else}
              <tr><td colspan="7" class="px-4 py-12 text-center text-[var(--muted)]">Sem abastecimentos ainda.</td></tr>
            {/each}
          </tbody>
        </table>
      </div>
    ...
    <div class="grid gap-4">
      <form class="panel grid gap-3 p-4" method="POST" action="?/createRecord" enctype="multipart/form-data" use:enhance>
        ...
        <input class="field" type="number" step="0.001" name="liters" placeholder="Litros" value={ocr?.liters ?? ''} required />
        <input class="field" type="number" step="0.01" name="total_price" placeholder="Valor total" value={ocr?.total_price ?? ''} required />
        <input class="field" type="number" step="0.001" name="price_per_liter" placeholder="Preço por litro opcional" value={ocr?.price_per_liter ?? (defaults.price_per_liter_millicents ? defaults.price_per_liter_millicents / 100000 : '')} />
        <select class="field" name="station_id"><option value="">Posto cadastrado</option>{#each data.stations as station}<option value={station.id} selected={defaults.station_id === station.id}>{station.name}</option>{/each}</select>
        <select class="field" name="fuel_grade_id"><option value="">Combustível cadastrado</option>{#each data.grades as grade}<option value={grade.id} selected={defaults.fuel_grade_id === grade.id}>{grade.name}</option>{/each}</select>
        <input class="field" name="station_name" placeholder="Nome do posto" value={defaults.station_name ?? ''} />
        <input class="field" name="fuel_type" placeholder="Tipo" value={defaults.fuel_type ?? 'gasoline'} />
        <label class="flex items-center gap-2 text-sm"><input type="checkbox" name="tank_full" value="true" checked={defaults.tank_full ?? true} /> Tanque cheio</label>
        <textarea class="field" name="notes" placeholder="Observações"></textarea>
        <input class="field" type="file" name="receipt_file" accept="image/*,.pdf,.txt" />
      ...
      <form class="panel grid gap-3 p-4" method="POST" action="?/repeatLast" use:enhance>
        ...
        <input class="field" type="number" step="0.01" name="total_price" placeholder="Valor total" required />
  ...
  <div class="grid gap-6 lg:grid-cols-2">
    <div class="panel p-4">
      ...
      <form class="mt-3 grid gap-3" method="POST" action="?/saveStation" use:enhance>
        ...
        <div class="grid gap-3 sm:grid-cols-2"><input class="field" name="city" placeholder="Cidade" /><input class="field" name="state" placeholder="UF" /></div>
        <textarea class="field" name="notes" placeholder="Observações"></textarea>
        <button class="button-secondary" type="submit">Salvar posto</button>
      </form>
      <div class="mt-4 grid gap-2">{#each data.stations as station}<div class="flex items-center justify-between border-t border-[var(--line)] py-2 text-sm"><span>{station.name}</span><form method="POST" action="?/deleteStation" use:enhance><input type="hidden" name="id" value={station.id} /><button class="text-danger">Excluir</button></form></div>{/each}</div>
    </div>
    ...
    <div class="panel p-4">
      ...
      <form class="mt-3 grid gap-3" method="POST" action="?/saveGrade" use:enhance>
        ...
        <div class="grid gap-3 sm:grid-cols-3"><input class="field" name="octane_rating" placeholder="Octanas" /><input class="field" name="ethanol_percentage" placeholder="% etanol" /><input class="field" name="default_price_per_liter" placeholder="Preço padrão" /></div>
        <textarea class="field" name="notes" placeholder="Observações"></textarea>
        <button class="button-secondary" type="submit">Salvar combustível</button>
      </form>
      <div class="mt-4 grid gap-2">{#each data.grades as grade}<div class="flex items-center justify-between border-t border-[var(--line)] py-2 text-sm"><span>{grade.name}</span><form method="POST" action="?/deleteGrade" use:enhance><input type="hidden" name="id" value={grade.id} /><button class="text-danger">Excluir</button></form></div>{/each}</div>
    </div>
  </div>
  ...
  <div class="grid gap-6 lg:grid-cols-2">
    <form class="panel grid gap-3 p-4" method="POST" action="?/saveDefaults" use:enhance>
      ...
      <input class="field" name="price_per_liter" placeholder="Preço por litro" />
      <label class="flex items-center gap-2 text-sm"><input type="checkbox" name="tank_full" value="true" checked /> Tanque cheio por padrão</label>
