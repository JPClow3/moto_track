...
export const actions = {
  ...
  revokeShare: async ({ request, locals }) => {
    const form = await request.formData();
    const { error } = await locals.supabase.from("sale_report_shares").update({ revoked_at: new Date().toISOString() }).eq("id", String(form.get("id") ?? "")).eq("owner_id", locals.user!.id);
...
export async function load({ locals, url }) {
  ...
  const timeline = buildTimeline(events, { source: url.searchParams.get("source") || undefined, start: url.searchParams.get("start") || undefined, end: url.searchParams.get("end") || undefined });
