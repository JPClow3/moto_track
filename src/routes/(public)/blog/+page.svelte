...
<section class="relative overflow-hidden border-b border-[var(--line)]">
  ...
  <div class="relative mx-auto max-w-6xl px-6 py-16 sm:py-20">
    ...
    <h1 class="display mt-5 max-w-3xl text-5xl sm:text-6xl lg:text-7xl">
      Guias que evitam<br /><span class="text-[var(--accent)]">oficina cara.</span>
    </h1>
    <p class="mt-6 max-w-xl text-lg leading-relaxed text-[var(--muted)]">
      Passo a passo, torque, intervalo e peça certa — escrito para quem faz a manutenção da própria
      moto.
    </p>
  </div>
</section>
...
<section class="px-6 py-16">
  <div class="mx-auto max-w-6xl">
    {#if lead}
      ...
      <a class="lead panel group relative block overflow-hidden p-8 sm:p-10" href={`/blog/${lead.slug}`}>
        ...
        <div class="relative max-w-3xl">
          ...
          <h2 class="display mt-4 text-3xl transition-colors group-hover:text-[var(--accent)] sm:text-5xl">
            {lead.title}
          </h2>
          ...
          <div class="mt-7 flex flex-wrap items-center gap-4">
            <span class="label-tech inline-flex items-center gap-2 text-[var(--accent)]">
              Ler guia
              <ArrowRight class="h-3.5 w-3.5 transition-transform group-hover:translate-x-1" />
            </span>
            <span class="label-tech text-[var(--muted)]">{formatDate(lead.published_at)}</span>
    ...
    {#if rest.length}
      <div class="mt-px grid gap-px border border-[var(--line)] bg-[var(--line)] sm:grid-cols-2 lg:grid-cols-3">
        {#each rest as article (article.slug)}
          <a class="card group flex flex-col bg-[var(--bg)] p-7" href={`/blog/${article.slug}`}>
            <div class="flex items-center gap-3">
              <span class="label-tech text-[var(--muted)]">{formatDate(article.published_at)}</span>
              <span class="label-tech inline-flex items-center gap-1.5 text-[var(--muted)]">
                <Clock class="h-3 w-3" />
                {article.reading_minutes} min
              </span>
            </div>
            <h3 class="display mt-5 text-2xl transition-colors group-hover:text-[var(--accent)]">
              {article.title}
            </h3>
    ...
    {#if !data.articles.length}
      <div class="panel p-12 text-center">
        ...
        <p class="mt-3 text-[var(--muted)]">Os primeiros guias de manutenção chegam em breve.</p>
      </div>
    {/if}
  </div>
</section>
