...
export const actions = {
  ...
  demo: async ({ locals }) => {
    const { error } = await locals.supabase.from("motorcycles").insert({ owner_id: locals.user!.id, name: "Moto de demonstração", brand: "Moto Track", model: "Demo", year: new Date().getFullYear(), current_odometer_km: 1200 });
    if (error) return fail(400, { message: error.message });
    throw redirect(303, "/dashboard");
  },
};
