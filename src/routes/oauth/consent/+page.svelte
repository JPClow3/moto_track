...
<svelte:head
  ><title>Autorizar {data.client.name} · Moto Track</title></svelte:head
>
...
>
  ...
  <div class="relative w-full max-w-[440px]">
    ...
    <div class="panel relative overflow-hidden p-6 shadow-lift sm:p-8">
      ...
      <div
        class="mb-6 flex items-center gap-3 rounded border border-[var(--line)] p-4"
      >
        ...
        <div class="min-w-0">
          ...
          {#if data.client.uri}
            <a
              ...
              target="_blank"
              rel="noreferrer"
              class="flex items-center gap-1 truncate text-xs text-[var(--muted)] hover:text-[var(--accent)]"
            >
      ...
      <p class="mb-6 text-xs text-[var(--muted)]">
        Você será redirecionado para <span class="break-all"
          >{data.redirectUri}</span
        >.
      </p>
