...
export const actions = {
  signIn: async ({ request, locals, url }: import('./$types').RequestEvent) => {
  ...
  signUp: async ({ request, locals, url }: import('./$types').RequestEvent) => {
    const form = await request.formData();
    const email = String(form.get("email") ?? "");
    const password = String(form.get("password") ?? "");
  ...
  google: async ({ locals, url }: import('./$types').RequestEvent) => {
  ...
  resetPassword: async ({ request, locals, url }: import('./$types').RequestEvent) => {
    const form = await request.formData();
    const email = String(form.get("email") ?? "");
