...
import { r as restoreMotorcycle, a as archiveMotorcycle } from "../../../../chunks/parity.js";
import { s as syncMotorcycleOdometer } from "../../../../chunks/odometer.js";
const FREE_ACTIVE_MOTORCYCLE_LIMIT = 1;
function hasProAccess(profile, now = /* @__PURE__ */ new Date()) {
...
async function load({ locals }) {
  const [motorcycles, profile] = await Promise.all([
    locals.supabase.from("motorcycles").select("*, motorcycle_specs(*)").eq("owner_id", locals.user.id).order("is_active", { ascending: false }).order("name"),
    locals.supabase.from("subscription_profiles").select("plan, stripe_subscription_status, grace_until").eq("owner_id", locals.user.id).maybeSingle()
  ]);
  return {
    motorcycles: motorcycles.data ?? [],
    canAddActive: hasProAccess(profile.data) || (motorcycles.data ?? []).filter((motorcycle) => motorcycle.is_active).length < FREE_ACTIVE_MOTORCYCLE_LIMIT
  };
}
const actions = {
  create: async ({ request, locals }) => {
    const form = await request.formData();
    const { count } = await locals.supabase.from("motorcycles").select("id", { count: "exact", head: true }).eq("owner_id", locals.user.id).eq("is_active", true);
    const { data: profile } = await locals.supabase.from("subscription_profiles").select("plan, stripe_subscription_status, grace_until").eq("owner_id", locals.user.id).maybeSingle();
  ...
  archive: async ({ request, locals }) => {
    const form = await request.formData();
    const { error } = await locals.supabase.from("motorcycles").update(archiveMotorcycle()).eq("id", value(form, "id")).eq("owner_id", locals.user.id);
    return error ? fail(400, { message: error.message }) : { ok: true };
  },
  restore: async ({ request, locals }) => {
    ...
    const { error } = await locals.supabase.from("motorcycles").update(restoreMotorcycle()).eq("id", value(form, "id")).eq("owner_id", locals.user.id);
    return error ? fail(400, { message: error.message }) : { ok: true };
  },
  ...
  updateOdometer: async ({ request, locals }) => {
    ...
    const { error } = await locals.supabase.from("motorcycles").update({ odometer_override_km: override, odometer_override_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", motorcycleId).eq("owner_id", locals.user.id);
