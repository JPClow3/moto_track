...
async function staffState(locals) {
  const { data: profile } = await locals.supabase.from("profiles").select("is_staff").eq("id", locals.user.id).maybeSingle();
  return Boolean(profile?.is_staff);
}
async function load({ locals, platform }) {
  ...
  const [
    ...
    supabase.from("account_data_requests").select("*", { count: "exact", head: true }),
    supabase.from("site_settings").select("*").eq("id", 1).maybeSingle(),
    supabase.from("forum_articles").select("id, title, slug, is_published, published_at").order("published_at", { ascending: false }).limit(10),
    supabase.from("motorcycle_templates").select("*").order("brand").order("model").limit(20),
    supabase.from("account_data_requests").select("*").order("created_at", { ascending: false }).limit(20)
  ]);
  return {
    isStaff,
    settings,
...
const actions = {
  fulfillDataRequest: async ({ request, locals, platform }) => {
    ...
    const { error } = await createSupabaseAdminClient(platform).from("account_data_requests").update({ status: "fulfilled", fulfilled_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", String(form.get("id") ?? ""));
  ...
  createArticle: async ({ request, locals, platform }) => {
    ...
    const slug = String(form.get("slug") ?? "") || title.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
    const supabase = createSupabaseAdminClient(platform);
  ...
  createTemplate: async ({ request, locals, platform }) => {
    ...
    const { error } = await supabase.from("motorcycle_templates").insert({
      ...
      year_from: Number(form.get("year_from") ?? 2e3),
