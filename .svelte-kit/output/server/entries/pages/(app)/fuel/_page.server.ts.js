...
import { p as parseFuelImportRows, b as parseFuelCsv, d as parseReceiptFile, c as costPerKm, a as averageConsumption } from "../../../../chunks/fuel.js";
import { u as uploadObjectFile } from "../../../../chunks/files.js";
import { r as runtimeEnv } from "../../../../chunks/runtime.js";
import { s as syncMotorcycleOdometer } from "../../../../chunks/odometer.js";
function cents(value) {
  const parsed = Number(String(value ?? "0").replace(",", "."));
  return Number.isFinite(parsed) ? Math.round(parsed * 100) : 0;
}
function millicents(value) {
  const parsed = Number(String(value ?? "0").replace(",", "."));
  return Number.isFinite(parsed) ? Math.round(parsed * 1e5) : 0;
}
...
async function createFuelRecord({
  ...
  const pricePerLiter = millicents(form.get("price_per_liter")) || (liters > 0 ? Math.round(totalCents / 100 / liters * 1e5) : 0);
  const payload = {
    id,
    owner_id: user.id,
    ...
    date: String(form.get("date") || (/* @__PURE__ */ new Date()).toISOString().slice(0, 10)),
  ...
  await locals.supabase.from("fuel_preferences").upsert(
    ...
    {
      onConflict: "owner_id,motorcycle_id,station_id,fuel_grade_id,fuel_type,station_name"
    }
  );
...
async function load({ locals }) {
  ...
  const [
    ...
    locals.supabase.from("fuel_records").select("*").eq("owner_id", user.id).order("date", { ascending: false }).order("odometer_km", { ascending: false }),
    locals.supabase.from("motorcycles").select("id, name, current_odometer_km").eq("owner_id", user.id).eq("is_active", true).order("name"),
    locals.supabase.from("fuel_stations").select("*").eq("owner_id", user.id).order("name"),
    locals.supabase.from("fuel_grades").select("*").eq("owner_id", user.id).order("name"),
    locals.supabase.from("fuel_preferences").select("*").eq("owner_id", user.id).order("last_used_at", { ascending: false }),
    locals.supabase.from("fuel_review_preferences").select("*").eq("owner_id", user.id)
  ]);
...
const actions = {
  ...
  deleteRecord: async ({ request, locals }) => {
    ...
    const { data: existing, error: readError } = await locals.supabase.from("fuel_records").select("motorcycle_id").eq("owner_id", locals.user.id).eq("id", id).maybeSingle();
    if (readError) return fail(400, { message: readError.message });
    const { error } = await locals.supabase.from("fuel_records").delete().eq("owner_id", locals.user.id).eq("id", id);
    if (error) return fail(400, { message: error.message });
  ...
  repeatLast: async ({ request, locals, platform }) => {
    const form = await request.formData();
    const { data: last, error } = await locals.supabase.from("fuel_records").select("*").eq("owner_id", locals.user.id).order("date", { ascending: false }).order("odometer_km", { ascending: false }).limit(1).maybeSingle();
    if (error || !last)
      return fail(400, {
    ...
    form.set(
      "price_per_liter",
      String(Number(last.price_per_liter_millicents ?? 0) / 1e5)
    );
  ...
  saveStation: async ({ request, locals }) => {
    ...
    const query = id ? locals.supabase.from("fuel_stations").update(payload).eq("id", id).eq("owner_id", locals.user.id) : locals.supabase.from("fuel_stations").insert(payload);
  ...
  deleteStation: async ({ request, locals }) => {
    const form = await request.formData();
    const { error } = await locals.supabase.from("fuel_stations").delete().eq("id", String(form.get("id") ?? "")).eq("owner_id", locals.user.id);
    if (error) return fail(400, { message: error.message });
    return { ok: true };
  },
  saveGrade: async ({ request, locals }) => {
    ...
    const query = id ? locals.supabase.from("fuel_grades").update(payload).eq("id", id).eq("owner_id", locals.user.id) : locals.supabase.from("fuel_grades").insert(payload);
    const { error } = await query;
    if (error) return fail(400, { message: error.message });
    return { ok: true };
  },
  deleteGrade: async ({ request, locals }) => {
    const form = await request.formData();
    const { error } = await locals.supabase.from("fuel_grades").delete().eq("id", String(form.get("id") ?? "")).eq("owner_id", locals.user.id);
    if (error) return fail(400, { message: error.message });
    return { ok: true };
  },
  ...
  saveReviewSettings: async ({ request, locals }) => {
    const form = await request.formData();
    const { error } = await locals.supabase.from("fuel_review_preferences").upsert({
      owner_id: locals.user.id,
