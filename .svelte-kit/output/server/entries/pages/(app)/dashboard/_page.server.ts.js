import { c as costPerKm, a as averageConsumption } from "../../../../chunks/fuel.js";
function dashboardSummary(fuelRecords) {
  return {
...
async function load({ locals }) {
  ...
  const [
    ...
    locals.supabase.from("motorcycles").select("*").eq("owner_id", user.id).eq("is_active", true),
    locals.supabase.from("fuel_records").select("*").eq("owner_id", user.id),
    locals.supabase.from("reminders").select("*").eq("owner_id", user.id).eq("is_active", true),
    locals.supabase.from("tire_records").select("*").eq("owner_id", user.id).eq("is_active", true),
    locals.supabase.from("motorcycle_documents").select("*").eq("owner_id", user.id)
  ]);
  const summary = dashboardSummary(fuel ?? []);
  const activeMotorcycle = motorcycles?.[0];
  ...
  const metrics = [
    ...
    {
      label: "Consumo médio",
      value: summary.average_consumption_km_l ? `${summary.average_consumption_km_l} km/L` : "Sem dados",
