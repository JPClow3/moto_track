...
async function hmac(keyBytes: Uint8Array, input: Uint8Array) {
  ...
  return new Uint8Array(await crypto.subtle.sign("HMAC", key, arrayBuffer(input)));
}
...
function vapidPrivateJwk(config: WebPushConfig) {
  ...
  const privateKey = config.privateKey.trim().startsWith("{")
    ...
    : { d: config.privateKey };
  if (publicKey.length !== 65 || publicKey[0] !== 4) throw new Error("PUBLIC_VAPID_KEY must be an uncompressed P-256 key.");
  return {
    ...privateKey,
    kty: "EC",
    crv: "P-256",
...
async function vapidToken(endpoint: string, config: WebPushConfig) {
  ...
  const payload = base64Url(encoder.encode(JSON.stringify({ aud: audience, exp: Math.floor(Date.now() / 1000) + 12 * 60 * 60, sub: config.subject })));
  const key = await crypto.subtle.importKey("jwk", vapidPrivateJwk(config), { name: "ECDSA", namedCurve: "P-256" }, false, ["sign"]);
...
async function encryptedPayload(subscription: WebPushSubscription, payload: string) {
  const clientPublicBytes = fromBase64Url(subscription.p256dh);
  const clientPublic = await crypto.subtle.importKey("raw", clientPublicBytes, { name: "ECDH", namedCurve: "P-256" }, false, []);
  const serverKeyPair = await crypto.subtle.generateKey({ name: "ECDH", namedCurve: "P-256" }, true, ["deriveBits"]);
  const sharedSecret = new Uint8Array(await crypto.subtle.deriveBits({ name: "ECDH", public: clientPublic }, serverKeyPair.privateKey, 256));
  ...
  const ikm = await hkdfExpand(
    ...
    join(encoder.encode("WebPush: info\0"), clientPublicBytes, serverPublicBytes),
    32,
  );
  ...
  return join(salt, recordSize, new Uint8Array([serverPublicBytes.length]), serverPublicBytes, ciphertext);
}

export async function sendWebPush(subscription: WebPushSubscription, payload: Record<string, string>, config: WebPushConfig) {
